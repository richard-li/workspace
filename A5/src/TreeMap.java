...
public class TreeMap {
    ...
    public static Wrapper2 getSplit(ArrayList<Node> b, int m, int n) {
        ...
        for (int i = m; i <= n ; i++) {
            totalWeight += b.get(i).getWeight();
        }
        if (totalWeight <= 0)
            return new Wrapper2((m + n), 0.5);

        double sum = b.get(m).getWeight();
        int k = m;
        while (sum < SPLIT_RATIO * totalWeight) {
            k++;
            sum += b.get(k).getWeight();
        }
        return new Wrapper2(k, sum / totalWeight);
    }
    ...
    private void sliceAndDice(ArrayList<Node> b, int m, int n,
        ...
        {
            Vector2D newLower = new Vector2D((bbox.lower.x + (bbox.upper.x - bbox.lower.x)*ratio), bbox.lower.y);
            Vector2D newUpper = new Vector2D((bbox.lower.x + (bbox.upper.x - bbox.lower.x)*ratio), bbox.upper.y);
            splitbox = new BoundingBox(bbox.lower, newUpper);
            restbox = new BoundingBox(newLower, bbox.upper);
        }
        else
        {
            Vector2D newLower = new Vector2D(bbox.lower.x, (bbox.lower.y + (bbox.upper.y - bbox.lower.y)*ratio));
            Vector2D newUpper = new Vector2D(bbox.upper.x, (bbox.lower.y + (bbox.upper.y - bbox.lower.y)*ratio));
            splitbox = new BoundingBox(bbox.lower, newUpper);
            restbox = new BoundingBox(newLower, bbox.upper);
        }
        ...
        //ArrayList<Node> splitlist = new ArrayList<Node>(b.subList(m, ksplit+1));
        //ArrayList<Node> restlist = new ArrayList<Node>(b.subList(ksplit+1, n));
        sliceAndDice(b, m, ksplit, splitbox, w, h);
        sliceAndDice(b, ksplit+1, n, restbox, w, h);
    }
